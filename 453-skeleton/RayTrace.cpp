...
Intersection Sphere::getIntersection(Ray ray){
	...
	if (d >= 0) {
		d = sqrt(d);
		float sol1 = (-b - d) / (2.f * a);
		float sol2 = (-b + d) / (2.f * a);

		if (sol1 <= sol2 && sol1 >= 0) {
			i.numberOfIntersections = 1;
			i.point = sol1 * ray.direction + ray.origin;
			i.normal = glm::normalize(i.point -centre);
		}
		else if (sol2 <= sol1 && sol2 >= 0) {
			i.numberOfIntersections = 1;
			i.point = sol2 * ray.direction + ray.origin;
			i.normal = glm::normalize(i.point - centre);
		}
	}
	...
	float t = (minz- ray.origin.z) / ray.direction.z;
	float step = -(maxz - minz) / 50;
	...
	for (int j = 0; j < 50; j++) {
		if ( glm::distance(t*ray.direction+ ray.origin, centre) <= radius ) {
			i.numberOfIntersections = 1;
			i.point = t * ray.direction;
			i.normal = glm::normalize(glm::vec3(i.point.x - centre.x, i.point.y - centre.y, i.point.z - centre.z));
			break;
		}
		t += step;
	}
